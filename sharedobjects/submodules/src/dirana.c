...
int g_src_active = SRC_NO_SOURCE;
...
/**   RETURN : OK or ERROR                             */
/** ************************************************* **/
//STATUS getAmplStatus(char * status)
//{
//   STATUS l_res = OK;
//
//   l_res = doDspAmpStatus(status);
//
//
//   return (l_res);
//}
...
STATUS setSource(DR_AUDIO_SOURCES_TYPES p_source )
{
   ...
   switch(p_source)
   {
      case SRC_TUNER:
      {
         l_res = SetTunerOnPrimaryStereo();
      }
      break;
      case SRC_CD:
      {
         l_res = SetI2SOnPrimaryStereo(2);
      }
      break;
      case SRC_MP3:
      {
      ...
      case SRC_CDC:
      {
         l_res = SetCdcOnPrimaryStereo();
      }
      break;
      case SRC_AUX:
      {
         l_res = SetAuxOnPrimaryStereo();
      }
      break;
      case SRC_PHONE:
      {
         l_res = SetPhoneOnPrimaryStereo();
      }
      break;
      case SRC_TTS:
      {
         l_res = SetI2SOnPrimaryStereo(3);
      }
      break;
	  
	  case SRC_TA_PTY:
	  case SRC_TTS_ON_AUX:
	  case SRC_AUX_CONVERGENCE:
      {
         //return ERROR;
		 l_res = ERROR;
      }
...
/*                                                                         */
/***************************************************************************/
STATUS SetLoudness( uint8 p_loudness )
{
	STATUS l_result = OK;
	int p_volume;
...
STATUS SetBalance( sint8 p_balance )   //!< the balance step (enum t_audio_balance_step)
{
	STATUS l_res = OK;
	uint16 l_value_left;
...
STATUS SetFader( sint8 p_fader )   //!< the fader step (enum t_audio_fader_step)
{
	STATUS l_res = OK;
...
/**   RETURN : OK or ERROR                             */
/** ************************************************* **/
//STATUS setChannelGain(int p_channel,
//                      int p_gain)
//{
//	STATUS l_res = OK;
//
...
//      switch (p_channel)
//	   {
//		   case AMP_FRONT_CHANNEL:
//		   {
//			   if( p_gain == LOW_GAIN12 )
//			   {
//				   l_res = setBitAmpliB0(0x10);
//			   }
//			   else if( p_gain == HIGH_GAIN26 )
//			   {
//				   l_res = delBitAmpliB0(0x10);
//			   }
//			   else
//			   {
//				   l_res = ERROR;
...
//		   }
//		   break;
//		   case AMP_REAR_CHANNEL:
//		   {
//			   if( p_gain == LOW_GAIN12 )
//			   {
//				   l_res = setBitAmpliB0(0x8);
//			   }
//			   else if( p_gain == HIGH_GAIN26 )
//			   {
//				   l_res = delBitAmpliB0(0x8);
//			   }
//			   else
//			   {
//				   l_res = ERROR;
...
/**   RETURN : OK or ERROR                             */
/** ************************************************* **/
//STATUS getChannelGain(int p_channel,
//                      int *p_gain)
//{
//	STATUS l_res = OK;
//	switch (p_channel)
...
STATUS playSin(int p_volume)
{
   ...
   if(l_res == OK)
   {
      dspWrite(ADSP_X_EasyP_Index,X_REG_SIZE,ADSP_EASYP_SinGen_SineOnPrim);
      usleep(DELAY_TIME);
   }

   if(l_res == OK)
   {
      dspWrite(ADSP_X_EasyP_Index,X_REG_SIZE,ADSP_EASYP_SinGen_Enable);
      usleep(DELAY_TIME);
   }

   if(l_res == OK)
   {
      dspWrite(ADSP_Y_SinGen_GL,Y_REG_SIZE,0x000800); /** volume del canale sinistro a -12.0 Db */
   }

   if(l_res == OK)
   {
      dspWrite(ADSP_Y_SinGen_GR,Y_REG_SIZE,0x000800); /** volume del canale sinistro a 0 Db*/
   }

   if(l_res == OK)
   {
      dspWrite(ADSP_Y_SinGen_aHbak,Y_REG_SIZE,0x0007EB); /** imposta la forma d onda*/
   }

   if(l_res == OK)
   {
      dspWrite(ADSP_Y_SinGen_aLbak,Y_REG_SIZE,0x0001FD); /** imposta la forma d onda*/
   }

   if(l_res == OK)
   {
      dspWrite(ADSP_Y_SinGen_Init,Y_REG_SIZE,0x0001fd);
      usleep(DELAY_TIME);
   }

   if(l_res == OK)
   {
      dspWrite(ADSP_Y_Mute_P,Y_REG_SIZE,0x000800);  /** disable MUTE on primary channel*/
   }

   if(l_res == OK)
   {
      dspWrite(ADSP_Y_Mute_S,Y_REG_SIZE,0x000800);  /** disable MUTE on secondary channel*/
   }

   g_src_active = SRC_NO_SOURCE;
...
/**   RETURN : OK or ERROR                             */
/** ************************************************* **/
STATUS SetI2SOnPrimaryStereo(int p_bus)
{
   ...
   switch(p_bus)
   {
      ...
      case 2:
      {
         ...
         l_value = ADSP_EASYP_SrcSw_I2S2onA;
      }
      break;
      case 3:
      {
...
{ 
   ...
   PrintMessage(RED_PRINT,"SetTunerOnPrimaryStereo: setting Tuner on primary stereo...");
	...
	if(l_res == OK )
    {
      l_res = dspWrite(ADSP_Y_Mute_P,Y_REG_SIZE,0x0000);
    }

   		if(l_res == OK )
   		    {
      l_res = dspWrite(ADSP_X_EasyP_Index,X_REG_SIZE,ADSP_EASYP_SrcSw_Tuner1onA);
   		    }
   ...
   if(l_res == OK )
   {
      l_res = dspWrite(ADSP_Y_Mute_P,Y_REG_SIZE,0x0800);
   }
...
STATUS set_frequency(int p_frequency, TYPE_BAND p_band, TYPE_SUB_BAND p_subAM_band)
{
	...
	int l_res;
//	unsigned char tuner_read[TUNER_REG_SIZE_READ] ={ 0x00 };
    set_LeafDiceCfg(p_frequency,p_band,  p_subAM_band);
...
int get_frequency()
{
	...
	char l_band = 0;
	char l_subband = 0;
	get_Freq_Band_fromTuner(&l_freq_res, &l_band,  &l_subband);
...
void get_band(TYPE_BAND p_band, TYPE_SUB_BAND p_subband)
{
	...
	get_Freq_Band_fromTuner(&l_freq_res, &p_band, &p_subband);
	PrintMessage(GREEN_PRINT,"get_band -- l_freq_res = %d \t l_current_band = %d  \t l_current_sub_band = %d ",l_freq_res,p_band, p_subband );

}

STATUS get_Freq_Band_fromTuner(int *p_freq, TYPE_BAND *p_band, TYPE_SUB_BAND *p_subband)//only main tuner
{
	  STATUS l_res = OK;
	  unsigned char tuner_read[TUNER_REG_SIZE_READ] ={ 0x00 };
	  l_res = dspTunerRead(TUNER_T1A_ADDRESS, TUNER_REG_SIZE_READ, &tuner_read[0]);
	  ...
	  PrintMessage(GREEN_PRINT,"get_Freq_Band_fromTuner --  p_freq = %d \t  p_band = %d \t p_subband = %d ", *p_freq, *p_band, *p_subband);
	  return l_res;
}

STATUS set_LeafDiceCfg(int p_freq, TYPE_BAND p_band, TYPE_SUB_BAND p_subAM_band)//only main tuner
{
